...
class ArithReassocPassTest < Minitest::Test
  ...
  def test_rewritten_instruction_order_non_literals_first_then_literal_then_opt_pluses
    ...
    assert_equal 6, Optimize::Passes::LiteralValue.read(after_intermediate, object_table: ot)
  ...
  def test_opt_plus_minus_collapses_leading_non_literal_chain
    ...
    refute_nil f.instructions.find { |i| Optimize::Passes::LiteralValue.read(i, object_table: ot) == 2 }
    loaded = RubyVM::InstructionSequence.load_from_binary(Optimize::Codec.encode(ir))
  ...
  def test_opt_mult_pools_literals_across_same_op_non_literal
    ...
    refute f.instructions.any? { |i| [2, 3, 4, 6].include?(Optimize::Passes::LiteralValue.read(i, object_table: ot)) },
  ...
  def test_opt_mod_prefix_two_literals_folds
    ...
    refute_nil f.instructions.find { |i| Optimize::Passes::LiteralValue.read(i, object_table: ot) == 1 }
    refute f.instructions.any? { |i| Optimize::Passes::LiteralValue.read(i, object_table: ot) == 7 }
    loaded = RubyVM::InstructionSequence.load_from_binary(Optimize::Codec.encode(ir))
  ...
  def test_opt_mod_post_non_literal_does_not_fold_accumulator
    ...
    lit_values = f.instructions.map { |i| Optimize::Passes::LiteralValue.read(i, object_table: ot) }.compact
