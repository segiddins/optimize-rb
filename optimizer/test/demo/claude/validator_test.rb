...
class ValidatorTest < Minitest::Test
  ...
  def test_structural_empty_for_clean_ir
    errors = Optimize::Demo::Claude::Validator.structural(answer_fn)
    assert_equal [], errors
  end

  def test_structural_reports_unknown_opcode
    fn = answer_fn
    fn.instructions << Optimize::IR::Instruction.new(opcode: :opt_fastmath, operands: [], line: nil)
    errors = Optimize::Demo::Claude::Validator.structural(fn)
  ...
  def test_structural_reports_wrong_arity_too_few
    fn = answer_fn
    fn.instructions << Optimize::IR::Instruction.new(opcode: :putobject, operands: [], line: nil)
    errors = Optimize::Demo::Claude::Validator.structural(fn)
    assert_equal 1, errors.size
  ...
  def test_structural_reports_wrong_arity_too_many
    fn = answer_fn
    fn.instructions << Optimize::IR::Instruction.new(opcode: :leave, operands: [99], line: nil)
    errors = Optimize::Demo::Claude::Validator.structural(fn)
    assert_equal 1, errors.size
  ...
  def load_envelope(fixture_path)
    ...
    Optimize::Codec.decode(iseq.to_binary)
  end
  ...
  def test_semantic_passes_on_clean_envelope
    ...
    errors = Optimize::Demo::Claude::Validator.semantic(envelope, cases: [["answer", 5]])
    assert_equal [], errors
  end

  def test_semantic_reports_wrong_return_value
    envelope = load_envelope(FIXTURE_PATH)
    errors = Optimize::Demo::Claude::Validator.semantic(envelope, cases: [["answer", 999]])
    assert_equal 1, errors.size
  ...
  def test_semantic_reports_runtime_error
    ...
    # Inject an unknown opcode so Optimize::Codec.encode raises before we
    ...
    answer_child.instructions << Optimize::IR::Instruction.new(opcode: :opt_fastmath, operands: [], line: nil)
    errors = Optimize::Demo::Claude::Validator.semantic(envelope, cases: [["answer", 5]])
  ...
  def test_semantic_reports_one_error_per_failing_case
    envelope = load_envelope(FIXTURE_PATH)
    errors = Optimize::Demo::Claude::Validator.semantic(envelope, cases: [
      ["answer", 5],
      ["answer", 999],
      ["answer", 777],
    ])
  ...
  def test_semantic_rejects_empty_cases
    ...
    assert_raises(ArgumentError) do
      Optimize::Demo::Claude::Validator.semantic(envelope, cases: [])
    end
  end
  ...
  def test_structural_reports_missing_final_leave
    fn = answer_fn
    fn.instructions = [
      Optimize::IR::Instruction.new(opcode: :putobject, operands: [5], line: nil),
    ]
    errors = Optimize::Demo::Claude::Validator.structural(fn)
  ...
  def test_structural_reports_stack_underflow
    fn = answer_fn
    fn.instructions = [
      Optimize::IR::Instruction.new(opcode: :pop, operands: [], line: nil),
      Optimize::IR::Instruction.new(opcode: :putobject, operands: [5], line: nil),
      Optimize::IR::Instruction.new(opcode: :leave, operands: [], line: nil),
    ]
    errors = Optimize::Demo::Claude::Validator.structural(fn)
  ...
  def test_structural_reports_leftover_on_stack
    ...
    fn.instructions = [
      ...
      Optimize::IR::Instruction.new(opcode: :putobject, operands: [1], line: nil),
      Optimize::IR::Instruction.new(opcode: :putobject, operands: [2], line: nil),
      Optimize::IR::Instruction.new(opcode: :leave, operands: [], line: nil),
    ]
    errors = Optimize::Demo::Claude::Validator.structural(fn)
  ...
  def test_structural_skips_arity_for_unknown_opcode
    fn = answer_fn
    fn.instructions << Optimize::IR::Instruction.new(opcode: :bogus, operands: [1, 2, 3], line: nil)
    errors = Optimize::Demo::Claude::Validator.structural(fn)
    assert_equal 1, errors.size
