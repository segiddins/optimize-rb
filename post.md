...
The cost of a wrong contract clause isn't a slowdown. It's a miscompile.

If a late `prepend` slides a module into `Integer`'s ancestor chain after load, every `1 + 2` the optimizer folded to `putobject 3` stays `3` — even if the prepended `+` would have returned `6`. A JIT deoptimizes on a guard failure; this has no guards, so there is no deopt. The `3` is a literal in the iseq now; it has forgotten where it came from. Reassign `SCALE = 12` halfway through a run and the folded `6` stays `6`. Mutate `ENV["FEATURE_X"]` after the snapshot is taken and the branch that was supposed to flip is already gone. Lie in an RBS signature — claim an `Integer` return that's sometimes `nil` — and an inlined `+` crashes on a receiver the optimizer proved couldn't exist. Every one of those is a bug in the program, not in the optimizer, but none of them existed before the optimizer ran.

Inlining erases the callee's frame from the backtrace — splice `point.translate(dx, dy)` into its caller and a `NoMethodError` inside the body surfaces in the caller's line range, on an instruction the caller didn't write. `TracePoint` stops seeing the inlined `:call` and `:return` events. `Coverage` stops marking the inlined lines. Dead-branch elimination is worse: the dead branch is gone, so `Coverage.result` reports its lines unexecuted and a coverage-gated CI pipeline fails the build for a branch the source still contains. Nothing here is a miscompile — the rewritten program is exactly what the contract said was legal — but every tool that reaches for the iseq sees a program the `.rb` file doesn't match.

The iseq binary format is pinned. A pass that matches on `opt_plus` and `setlocal_WC_0` is pinned to whatever 4.0 called those and whatever operands they took; 4.1 is free to rename, reshape, or retire any of them, and the loader rejects a stale binary immediately rather than running it wrong. Every minor release is a porting pass. YJIT and ZJIT pay this cost at boot, from source. A pipeline that stamps binaries pays it directly, by hand.

Back to the loop that got nothing in §5. A peephole is exactly as powerful as the window it looks through, and a small forward-walking window cannot see a backedge. Loop-invariant hoisting, zero-trip elimination, iteration-count reasoning — the entire category of transformations that would make `sum_of_squares` interesting — want a CFG analysis and a notion of def-use, and either one is strictly more machinery than everything else in the optimizer put together. The honest answer to "why didn't the loop get faster" is that a peephole can't see a loop. The honest answer to "what would it take" is a different project.
